...
__global__ void space_step(float *sol, int cur_t, int next_t, int Nx, int maxT, double dt, double s, bool liquid,
			   float influx, int num_cells_absorb, float amount_per_cell, bool *boundary_edge, Cell *cells,
	...
	if (idx == 1) {
		if (liquid == true) {
			...
			sol[next] = sol[cur] +
				    ...
				    dt*(fmaxf(0, in) - cells[idx].consumption);
		} else
			sol[next] = sol[cur] +
				    s*(10*sol[cur-1] -
				    4*sol[cur+1] + 14*sol[cur+2] - 6*sol[cur+3] + sol[cur+4] - 15*sol[cur]) +
				    dt*(fmaxf(0, influx - absorbed) - cells[idx].consumption);
	} else if (idx == Nx-2) {
		if (liquid == true) {
			int in = 0;
			if (boundary_edge[idx] == true) in = influx - absorbed;
...
struct CarcinogenPDE {
	...
	CarcinogenPDE(int space_size, int num_timesteps, double diff, bool liq) {
		N = space_size;
		T = num_timesteps + 1;
		T_scale = 16.0;
		diffusion = diff;
	...
	void time_step(int step, Cell *cells, curandState_t *states) {
		...
		for (int n = 0; n < maxT-1; n++) {
			int next = (n+1)*Nx;
			int cur = n*Nx;
			space_step<<< blocks, threads >>>(sol, cur, next, Nx, maxT, dt, s, liquid, influx,
							  num_cells_absorb, amount_per_cell, boundary_edge, cells, states);
			CudaCheckError();
			CudaSafeCall(cudaDeviceSynchronize());
		}
